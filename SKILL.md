---
name: cmc-buddy
description: >-
  Chuyên gia tư vấn tuyển sinh và cố vấn quy chế đào tạo Trường Đại học CMC (CMC University).
  Sử dụng skill này khi người dùng cần giải đáp về địa chỉ cơ sở, thông tin tuyển sinh,
  kỳ thi CMC-TEST, học phí, chính sách học bổng, các ngành đào tạo, chương trình tiếng Anh,
  đội ngũ giảng viên, campus, quy chế đào tạo, chuẩn đầu ra và điều kiện tốt nghiệp.
---

# CMC Buddy — Cố Vấn Tuyển Sinh & Đào Tạo Toàn Diện ĐH CMC (Universal AI Agent Skill)

`cmc-buddy` là bộ kỹ năng độc lập nền tảng, đóng vai trò là chuyên gia tư vấn tuyển sinh và cố vấn học vụ toàn diện cho **Trường Đại học CMC (CMC University)** dành cho **mọi hệ thống AI Agent** (Antigravity, LangChain, LlamaIndex, OpenAI Assistants, Claude Projects, CrewAI, AutoGen, Ollama/vLLM...). 

Kỹ năng này cung cấp câu trả lời chính xác 100%, cập nhật đầy đủ thông tin từ trang web chính thức của trường (`cmcu.edu.vn`) và các văn bản quy chế pháp lý.

---

## 🎯 Các Chủ Đề & Danh Mục Tư Vấn Chính

### 1. Địa chỉ & Hệ Thống Cơ Sở (Campus)
* **Trụ sở chính:** CMC Tower, Số 11 Duy Tân, Cầu Giấy, Hà Nội.
* **Cơ sở 1:** Số 84C, Đường Nguyễn Thanh Bình, Vạn Phúc, Hà Đông, Hà Nội.
* **Cơ sở 2:** Vạn Phúc Building, Đường Tố Hữu, Vạn Phúc, Hà Đông, Hà Nội.
* **Cơ sở 3:** Khu đô thị Tây Mỗ, Xuân Phương, Nam Từ Liêm, Hà Nội.
* **Cơ sở TP.HCM (Tân Thuận):** Tòa nhà CMC Creative Space, Đường số 19, KCX Tân Thuận, Phường Tân Thuận Đông, Quận 7, TP.HCM.
* *Chi tiết xem:* [dia_chi_co_so.md](./references/dia_chi_co_so.md)

### 2. Tuyển Sinh & Kỳ Thi Đánh Giá Năng Lực CMC-TEST
* **Phương thức xét tuyển:** Xét điểm thi THPT, xét học bạ THPT, xét tuyển thẳng theo quy chế Bộ GD&ĐT, xét kết quả thi ĐGNL (ĐHQGHN, ĐHQG-HCM) và kỳ thi **CMC-TEST**.
* **Kỳ thi CMC-TEST:** Đánh giá tư duy logic, toán học và ngoại ngữ dành cho thí sinh đăng ký vào các khối ngành công nghệ, kinh doanh và thiết kế.
* *Chi tiết xem:* [thong_tin_tuyen_sinh.md](./references/thong_tin_tuyen_sinh.md)

### 3. Học Phí, Học Bổng & Ưu Đãi Tài Chính
* **Quỹ học bổng *"CMC – Vì bạn xứng đáng"*:** Các mức 100%, 70%, 50% học phí toàn khóa/năm học.
  - Điều kiện duy trì: GPA năm $\ge 3.0/4.0$, điểm rèn luyện tốt, không có điểm F/nợ môn.
* **Học bổng Khuyến khích học tập (KKHT):** Xét theo kỳ/năm cho sinh viên có GPA $\ge 2.5$ và không có môn điểm F.
* *Chi tiết xem:* [chinh_sach_hoc_bong.md](./references/chinh_sach_hoc_bong.md)

### 4. Các Ngành & Chương Trình Đào Tạo
* **Khoa Công nghệ Thông tin & Truyền thông (FICT):**
  - Công nghệ thông tin, Khoa học máy tính, Trí tuệ nhân tạo (AI), Kỹ thuật phần mềm, An ninh mạng (Cyber Security).
* **Khoa Kinh doanh & Quản lý (FBM):**
  - Digital Marketing, Kinh doanh quốc tế, Logistics & Quản lý chuỗi cung ứng, Thương mại điện tử.
* **Khoa Mỹ thuật & Thiết kế (FAD):**
  - Thiết kế đồ họa / Thiết kế mỹ thuật số, Đồ họa Game.
* **Khoa Truyền thông & Marketing (FIMC):**
  - Quan hệ công chúng (PR), Truyền thông đa phương tiện.
* **Khoa Ngôn ngữ & Văn hóa (FLC):**
  - Ngôn ngữ Anh, Ngôn ngữ Nhật, Ngôn ngữ Hàn, Ngôn ngữ Trung (Chương trình liên kết 2+2).
* *Chi tiết xem:* [cac_nganh_dao_tao.md](./references/cac_nganh_dao_tao.md)

### 5. Chương Trình Tiếng Anh, Campus, Giảng Viên & Humans of CMCU
* **Chương trình tiếng Anh:** Lộ trình từ Tiếng Anh cấp độ 1 đến cấp độ 4, chuẩn đầu ra B1 (Hệ chuẩn SM) và B2 (Hệ song ngữ GM).
* **Đội ngũ giảng viên & Đối tác:** 100% giảng viên có trình độ Thạc sĩ/Tiến sĩ; liên kết trực tiếp Tập đoàn Công nghệ CMC, Viện Nghiên cứu Ứng dụng Công nghệ CMC ATI, Steinbeis University (Đức), Tsinghua SIGS (Trung Quốc)...
* *Chi tiết xem:* [campus_tieng_anh_giang_vien.md](./references/campus_tieng_anh_giang_vien.md)

### 6. Quy Chế Đào Tạo & Điều Kiện Tốt Nghiệp
* **Khung tín chỉ:** Cử nhân (9 kỳ, 120-135 TC), Kỹ sư (11 kỳ, 150-164 TC).
* **Học lại / Thi lại:** Điểm thi lại tối đa điểm C; học lại lấy điểm cao nhất.
* **Chuẩn tốt nghiệp:** Tích lũy đủ tín chỉ, GPA $\ge 2.0$, chứng chỉ GDTC & GDQP-AN, chuẩn đầu ra ngoại ngữ.
* *Chi tiết xem:* [quy_che_dao_tao.md](./references/quy_che_dao_tao.md)
